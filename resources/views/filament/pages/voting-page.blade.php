<x-filament-panels::page>
                    ...
                    <div class="p-6">
                        <dl class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <dd class="mt-1 text-sm text-gray-900 dark:text-white">{{ $currentMember->full_name }}</dd>
                            </div>
                            <div>
                                <dt class="text-sm font-medium text-gray-500 dark:text-gray-400">Member Code</dt>
                                <dd class="mt-1 text-sm text-gray-900 dark:text-white">{{ $currentMember->code }}</dd>
                            </div>
                            <div>
                                <dt class="text-sm font-medium text-gray-500 dark:text-gray-400">Branch</dt>
                                <dd class="mt-1 text-sm text-gray-900 dark:text-white">{{ $currentMember->branch->branch_name ?? 'N/A' }}</dd>
                            </div>
                            <div>
                                <dt class="text-sm font-medium text-gray-500 dark:text-gray-400">MIGS Status</dt>
                                <dd class="mt-1">
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $currentMember->is_migs ? 'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-200' : 'bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-200' }}">
                                        {{ $currentMember->is_migs ? 'MIGS Member' : 'Not MIGS Member' }}
                                    </span>
                                </dd>
                            </div>
                            <div>
                                ...
                                <dd class="mt-1">
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $currentMember->share_amount > 0 ? 'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-200' : 'bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-200' }}">
                                        ₱{{ number_format($currentMember->share_amount, 2) }}
                                    </span>
                                </dd>
                            </div>
                            <div>
                                ...
                                <dd class="mt-1">
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $currentMember->process_type === 'Updating and Voting' ? 'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-200' : 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900 dark:text-yellow-200' }}">
                                        {{ $currentMember->process_type ?? 'Not Set' }}
                                    </span>
                                </dd>
                            </div>
                        </dl>
    ...
    @elseif($hasAlreadyVoted)
        {{-- View for users who have already voted --}}
        <div class="space-y-6">
            {{-- Success Banner --}}
                ...
                <div class="flex items-center gap-4">
                    ...
                    <div class="flex-1">
                        ...
                        <p class="text-green-100 mt-1">
                            Thank you for participating! Here are your votes:
                        </p>
                        @if($controlNumber)
            ...
            <div class="bg-blue-50 dark:bg-blue-950 rounded-lg p-6">
                <div class="flex items-start gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-sm text-blue-700 dark:text-blue-300 mt-1">
                            You can view your submitted votes above. Changes are no longer possible after submission.
                        </p>
                    </div>
                </div>
            </div>
    ...
    @else
        {{-- Voting interface for eligible users who haven't voted yet --}}
        <div class="space-y-6">
                ...
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-primary-100 mt-1">
                            {{ $this->getTotalSelectedVotes() }} of {{ $this->getTotalAvailableVotes() }} votes cast
                        </p>
                    </div>
                    <div class="text-right">
                        <div class="text-4xl font-bold">
                            {{ round(($this->getTotalSelectedVotes() / max($this->getTotalAvailableVotes(), 1)) * 100) }}%
                        </div>
                        ...
                        <div class="flex items-center justify-between">
                            <div>
                                ...
                                <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">
                                    Vote for up to {{ $position['vacant_count'] }} candidate(s)
                                </p>
                            </div>
                            <div class="text-right">
                                <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium
                                    {{ $this->getVoteCount($position['id']) === $position['vacant_count']
                                        ? 'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-200'
                                        : 'bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-300' }}">
                                    {{ $this->getVoteCount($position['id']) }} / {{ $position['vacant_count'] }} selected
                                </span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="p-6">
                        @if(count($position['candidates']) > 0)
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                @foreach($position['candidates'] as $candidate)
                                    @php
                                        $isSelected = $this->isSelected($position['id'], $candidate['id']);
                                    @endphp

                                    <div
                                        wire:click="toggleVote('{{ $position['id'] }}', '{{ $candidate['id'] }}')"
